...
export function upsertEnvVars(values: Record<string, string>): Set<string> {
  ...
  try {
    ...
    // Fork (vosburg-auto): always owner-only — .env holds bot tokens. Upstream
    // copies the old mode across, which preserves a legacy world-readable 0644.
...
export function removeEnvVar(key: string): { existed: boolean } {
  ...
  writeSecretEnvFile(envFile, content.replace(lineRegex, ''));
  return { existed: true };
}
