...
describe('migrations', () => {
  ...
  it('does not re-run the fork migration on a DB that applied it under its pre-sync number', async () => {
    const db = await initSqliteTestDb();
    ...
    await runMigrations(db, preSync);
    await runMigrations(db, [{ ...forkAutoCompactWindow, version: 16 }]);

    const before = await db.all<{ name: string }>('SELECT name FROM schema_version');
    ...
    const after = await db.all<{ name: string }>('SELECT name FROM schema_version');
    expect(after.filter((r) => r.name === 'auto-compact-window')).toHaveLength(1);
    expect(after).toHaveLength(before.length);
  });

  it('survives a drifted fork-migration name without crash-looping the host', async () => {
    ...
    await runMigrations(db);
    await expect(
      runMigrations(db, [{ ...forkAutoCompactWindow, name: 'auto-compact-window-renamed' }]),
    ).resolves.toBeUndefined();
    const cols = sqliteRaw(db).prepare("PRAGMA table_info('container_configs')").all() as { name: string }[];
