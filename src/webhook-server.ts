...
function ensureServer(): void {
  ...
  // Port: upstream's getWebhookPort (process env, then .env, validated).
  // Bind: fork patch — loopback unless WEBHOOK_BIND opts in (resolveListenConfig).
  const port = getWebhookPort();
  const { bind } = resolveListenConfig(process.env);
  const id = randomUUID();
